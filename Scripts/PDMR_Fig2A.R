# script to generate oncoplot of PDX
...
mat <- read.csv("../DATA/oncoplot_data_pdx.csv", check.names = F, stringsAsFactors = F)
...
clinical_input <- read.csv("../DATA/Supp_tables_pdx_metadata.csv", check.names = F, stringsAsFactors = F)
...
top_ann <-HeatmapAnnotation(
  ...
  barplot = anno_barplot(
    ...
    axis = TRUE,
    border=FALSE,
    ylim=c(0,40), # This is set to 40 max to show the variation in sample which are between 20 and 80, there are samples with TMB >200 and that suppresses the whole scale 
...
pdf("Figure_oncoplot_PDX.pdf",width = 10,height = 10)
...
decorate_annotation(
  "ONCOTREE",{
    grid.text("Tumor Histology", unit(-13, "mm"), just = "bottom",gp=gpar(fontsize=8))
  }
)
