...
cmd="--latency-wait 120 --directory $WORK_DIR --snakefile $SNAKEFILE --configfile $SAM_CONFIG --jobscript $NGS_PIPELINE/scripts/jobscript.sh --jobname {params.rulename}.{jobid} --nolock  --ri -k -p -r -j 2000 --resources objget=100 objput=20 objstoreRSEM=50 objstoreSTAR=50 --stats ngs_pipeline_${NOW}.stats -R rnaseq_final"
