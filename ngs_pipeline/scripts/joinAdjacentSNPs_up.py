...
def getReference(chrom, start, end, ref2bit):
    ...
    # things.
    ref = "".join(step_remove_header.communicate()[0].decode("utf-8").rstrip('\n\r').splitlines())
...
def examineAdjacent(adjacentVar, inBam, ref2bit):
    ...
    if len(correctedVars) == 0:
        ...
        #fixed
        #bamData=step_cut.communicate()[0].encode().rstrip('\n\r').split('\n')
        bamData=step_cut.communicate()[0].decode("utf-8").rstrip('\n\r').split('\n')
        #print (type(bamData))
        if len(bamData[0]) > 0:
...
def main(distance, inFile, inBam, ref2bit, outFile):
    ...
    while True:
        ...
        if line.startswith("##INFO=<ID=VT,"):
            ...
            #outFile.write(str('##INFO=<ID=VT,Number=1,Type=String,Description="Variant type, can be SNP, MNP, INS or DEL">'))
            print ('##INFO=<ID=VT,Number=1,Type=String,Description="Variant type, can be SNP, MNP, INS or DEL">',file=outFile)
            ...
            print ('##joinAdjacentSNPs_command=%s' % " ".join(map(str, sys.argv)),file=outFile)
            print ('##INFO=<ID=JOINED,Number=0,Type=Flag,Description="Variant is the result of joining adjacent SNPs/MNPs. Please use caution when interpreting sample-specific values.">',file=outFile)
            print ('##INFO=<ID=ALERT,Number=0,Type=Flag,Description="Variant is adjacent to other variants, but case was too complex for joinAdjacentSNPs to handle. Please review.">',file=outFile)
            print (line,file=outFile)
            ...
            if nextLine:
                ...
                if chrom == nextChrom and end >= nextPos - distance:
                    ...
                    if len(adjacentVar) == 0:
                        ...
                        print ("\n".join(map(str, correctedVars)),file=outFile)
